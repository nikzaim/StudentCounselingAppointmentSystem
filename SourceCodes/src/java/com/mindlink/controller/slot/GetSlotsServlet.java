 ...
 * @author nikza
 */
@WebServlet(name = "GetSlotsServlet", urlPatterns = {"/manage-slots"})
public class GetSlotsServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) 
        ...
        // 2. Fetch Data using DAO
        SlotDAO slotDAO = new SlotDAO();
        List<Slot> slotList = slotDAO.getSlotsByUserId(user.getId());

        // 3. Attach data to the request object
        request.setAttribute("slotList", slotList);

        // 4. Forward to the JSP
        // Note: The URL in the browser remains "/manage-slots"
        request.getRequestDispatcher("manage-slots.jsp").forward(request, response);
    }

    @Override
    protected void doPost(HttpServletRequest request, HttpServletResponse response) 
            throws ServletException, IOException {
        // Redirect POST requests to GET logic
