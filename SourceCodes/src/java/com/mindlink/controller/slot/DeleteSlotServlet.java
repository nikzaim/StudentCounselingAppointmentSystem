 ...
 * @author nikza
 */
@WebServlet(name = "DeleteSlotServlet", urlPatterns = {"/DeleteSlotServlet"})
public class DeleteSlotServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet DeleteSlotServlet</title>");            
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet DeleteSlotServlet at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) 
        ...
        boolean success = dao.deleteSlot(user.getId(), slotId);

        if (success) {
            response.sendRedirect("manage-slots?status=deleted");
        } else {
